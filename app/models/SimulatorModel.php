...
class SimulatorModel extends System_Model
{
    ...
    public function simulateMatch($wrestler1, $wrestler2, $isBulk = false)
    {
        ...
        if (!$isBulk) $this->logMessage("--- Match Start: {$wrestler1->name} vs. {$wrestler2->name} ---");
        
        // **THE FIX:** Use wrestler_id to access the match state array
        if (!$isBulk) $this->logMessage("{$wrestler1->name} starts with {$matchState[$wrestler1->wrestler_id]['hp']} HP and {$matchState[$wrestler1->wrestler_id]['stamina']} Stamina.");
        if (!$isBulk) $this->logMessage("{$wrestler2->name} starts with {$matchState[$wrestler2->wrestler_id]['hp']} HP and {$matchState[$wrestler2->wrestler_id]['stamina']} Stamina.");

        while ($turn < $this->maxTurns && $winner === null) {
            $turn++;
            if (!$isBulk) $this->logMessage("--- Turn {$turn} ---");
            ...
            if (!$isBulk) $this->logMessage("{$attackerState['data']->name} is on the offensive!");

            $move = $this->selectMove($attackerState);
            if (!$move) {
                if (!$isBulk) $this->logMessage("{$attackerState['data']->name} is too tired to perform a move and catches their breath.");
            ...
            if ($roll <= $hitChance) {
                ...
                if (!$isBulk) $this->logMessage("{$attackerState['data']->name} hits a {$move->move_name} for {$damage} damage!");
                if (!$isBulk) $this->logMessage("{$defenderState['data']->name} has {$defenderState['hp']} HP remaining.");

            } else {
                if (!$isBulk) $this->logMessage("{$attackerState['data']->name} goes for a {$move->move_name}... but {$defenderState['data']->name} reverses it!");
            ...
            if ($defenderState['hp'] <= 0) {
                if (!$isBulk) $this->logMessage("{$defenderState['data']->name} has been pinned!");
                $winner = $attackerState['data'];
            }
        }

        if ($winner) {
            if (!$isBulk) $this->logMessage("--- Match End: The winner is {$winner->name}! ---");
        } else {
            if (!$isBulk) $this->logMessage("--- Match End: The time limit has expired. It's a draw! ---");
            $winner = 'draw';
        }
    ...
    private function calculateHitChance($attackerState, $defenderState, $move)
    {
        ...
        $staminaPenalty = 0;
        if ($staminaPercentage < 0.3) $staminaPenalty = 20;
        else if ($staminaPercentage < 0.5) $staminaPenalty = 10;
