...
class Challenge extends BaseModel
{
    ...
    private function sendChallengeEmails($challenger_pid, $defender_pid, $wager_amount)
    {
        ...
        // Send to defender
        $subject  = "You have a new challenge on IWF!";
        $htmlBody = "<p>{$challengerProspect['name']} has challenged your prospect {$defenderProspect['name']} for {$wager_amount} Gold.</p>
                 <p><a href='" . $_ENV['APP_URL'] . "/challenge/manage'>View Challenge</a></p>";
        try {
            $this->mailer->send($defenderUser['email'], $subject, $htmlBody);
        } catch (\Exception $e) {
            error_log("Failed to send challenge email to defender: " . $e->getMessage());
        }

        // Confirmation to challenger
        $subject2  = "Your IWF challenge has been sent!";
        $htmlBody2 = "<p>You challenged {$defenderProspect['name']} for {$wager_amount} Gold.</p>
                  <p>You will be notified when they respond.</p>";
        try {
            $this->mailer->send($challengerUser['email'], $subject2, $htmlBody2);
        } catch (\Exception $e) {
            error_log("Failed to send confirmation email to challenger: " . $e->getMessage());
        }
    }
}
