...
class TournamentController extends BaseController
{
     ...
     */
    public function __construct(Environment $twig, private Api $apiModel, private Simulator $simModel)
    {
    ...
    /**
     * @param string|null $type The type of tournament to load (standard, giants, etc.)
     * @return mixed
     */
    public function index($type = null)
    {
        // 1. Handle Framework Injection:
        // If $type is an object (e.g., a Request object injected by the router),
        // we check if the actual parameter was passed as the second argument.
        if (is_object($type)) {
            $args = func_get_args();
        ...
        // 2. Validate Type:
        // Whitelist valid types. If null or invalid, show the selection screen.
        ...
        if ($type === null || ! in_array($type, $validTypes)) {
            return $this->view(
                ...
                [
                    ...
                    'matchups'     => [], // Empty matchups triggers the selection UI in the view
                ]
            );
        }
        ...
        switch ($type) {
            ...
            case 'aerialists':
                return $this->aerialists();
            case 'strongmen':
                return $this->strongmen();
            default:
                // Fallback to selection if type is invalid
    ...
    {
        // Fetch all wrestlers and pick 32 random ones
        $model     = $this->model('Api');
        $wrestlers = $model->get_all_wrestlers();

        // Filter out incomplete data if necessary, or just shuffle
        shuffle($wrestlers);

        // Take top 32
        $tournament_wrestlers = array_slice($wrestlers, 0, 32);

        return $this->render_tournament_view(
            'Standard Tournament',
            'No Restrictions',
            $tournament_wrestlers
        );
    }
    ...
    {
        $simulatorModel   = $this->simModel;
        $simulation_count = 100;
        $wrestler1_wins   = 0;

        for ($i = 0; $i < $simulation_count; $i++) {
            // Use array syntax for IDs
            $result = $simulatorModel->run($wrestler1['wrestler_id'], $wrestler2['wrestler_id']);

            // Simulator returns an object or array for winner?
            // Usually returns array with 'winner' object. Let's check safely.
            if (isset($result['winner'])) {
                $winnerId = is_object($result['winner']) ? $result['winner']->wrestler_id : $result['winner']['wrestler_id'];
        ...
        try {
            ...
            foreach ($matchups as $match) {
                ...
                if ($wrestler1 && $wrestler2) {
                    ...
                    $odds_data[] = [
                        'wrestler1' => ['name' => $wrestler1['name']], // Fixed array syntax
                        'wrestler2' => ['name' => $wrestler2['name']], // Fixed array syntax
                        'odds'      => $odds,
                    ];
                }
            }
    ...
    public function start()
    {
        ...
        try {
            ...
            if (empty($wrestler_ids)) {
                ...
                $wrestler_ids         = array_map(fn($w) => $w['wrestler_id'], $tournament_wrestlers); // Fixed array syntax
            }
    ...
    public function simulate()
    {
        ...
        try {
            ...
            foreach ($matchups as $index => $match) {
                ...
                // Handle result object/array safely
                if (isset($simulationResult['winner']) && is_object($simulationResult['winner'])) {
                    $winnerId = $simulationResult['winner']->wrestler_id;
                } elseif (isset($simulationResult['winner']) && is_array($simulationResult['winner'])) {
                    $winnerId = $simulationResult['winner']['wrestler_id'];
                } else {
                    $winnerId = $match[rand(0, 1)]; // Fallback
                }
    ...
    /**
     * Helper to fetch data and call render
     */
    private function create_themed_tournament($title, $slogan, $fetch_method, $value)
    {
        ...
        // Use the common render method
        return $this->render_tournament_view($title, $slogan, $wrestlers);
    }

    /**
     * Renders the tournament view given a list of wrestlers.
     * Logic extracted from create_themed_tournament to be reusable.
     */
    ...
    // payToContinue method remains largely the same but ensure IDs are handled as expected if modified
    // (omitted for brevity if unchanged, but included below for completeness)
        ...
        try {
            ...
            foreach ($matchups as $match) {
                ...
                if (isset($simulationResult['winner'])) {
                    ...
                    $w               = $simulationResult['winner'];
                    $actualWinners[] = is_object($w) ? $w->wrestler_id : $w['wrestler_id'];
                }
            }
            ...
            if ($stmt->rowCount() === 0) {
                ...
                // Sometimes rowCount is 0 if data didn't change, but here it should change.
            }
